...
#Vistas endogenas.
class ListadodeProductos(LoginRequiredMixin, View):
    login_url = '/inventario/login'
    redirect_field_name = None

    form_class =SeleCategoriaForm
    template_name = 'inventario/producto/select_categoria.html'
    ...
    def post(self, request, *args, **kwargs):                                   # gets selected supplier and redirects to 'PurchaseCreateView' class
        ...
        if form.is_valid():
                categoriarid = request.POST.get("category")
                Activoid = request.POST.get("Activo")
                Tieneiva = request.POST.get("tiene_iva")
                ...
                if Activoid=='true':
                           Activoid=True
                           literalactivo='Productos Activos'
                else:
                           Activoid=False
                           literalactivo='Productos Anulados'

                if Tieneiva =='true':
                           Tieneiva =True
                           literaliva='Marcado con Iva'
                else:
                           Tieneiva =False
                           literaliva='Marcado Exento'
                if  nivel=='1':
                            literalNivel='Con Existencia en Cero'
                            if deposito=='':
                                producto = Producto.objects.filter(category_id=categoriarid,
                                Activo=Activoid,
                                tiene_iva=Tieneiva,
                                disponible=0)
                            else:
                                literalDeposito='Ubicacion ' +deposito
                                producto = Producto.objects.filter(category_id=categoriarid,
                                Activo=Activoid,
                                tiene_iva=Tieneiva,
                                disponible=0,
                            ...
                            literalNivel='Bajo Nivel Minimo'
                            if deposito=='':
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible <=p.MinUnidades')
                            else:
                                literalDeposito='Ubicacion ' +deposito
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible <=p.MinUnidades and p.Deposito= %s', [deposito])

                elif  nivel=='4':
                            literalNivel='Sobre Nivel Maximo'
                            if deposito=='':
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible >p.MaxUnidades')
                            else:
                                literalDeposito='Ubicacion ' +deposito
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])


                categoria = Categoria.objects.filter(id=categoriarid)
                contexto = {'producto':producto,
                            'categoria':categoria,
                            'literalactivo': literalactivo,
                            'literaliva':literaliva,
                            'literalNivel':literalNivel,
                            'literalDeposito':literalDeposito,
                            'categoriarid':categoriarid,
                            'Activoid':Activoid,
                            'Tieneiva':Tieneiva,
                            'deposito':deposito,
                            'nivel':nivel
                                                        }
                RPdf= ReporteProdPdf.objects.get(id=1)
                RPdf.categoriarid = categoriarid
                RPdf.Activoid = Activoid
                RPdf.Tieneiva = Tieneiva
                RPdf.deposito= deposito
                RPdf.nivel =nivel
                RPdf.save()
                contexto = complementarContexto(contexto,request.user)
                return render(request, 'inventario/producto/ReporteInvCategoria.html',contexto)

        return render(request, self.template_name, {'form': form})


class ListadodeProductosMarca(LoginRequiredMixin, View):
    login_url = '/inventario/login'
    redirect_field_name = None

    form_class =SeleMarcaForm
    template_name = 'inventario/producto/select_marca.html'
    ...
    def post(self, request, *args, **kwargs):                                   # gets selected supplier and redirects to 'PurchaseCreateView' class
        form = self.form_class(request.POST)
        if form.is_valid():
                marcaid = request.POST.get("marca")
                Activoid = request.POST.get("Activo")
                Tieneiva = request.POST.get("tiene_iva")
                deposito = request.POST.get("Deposito")
                nivel =request.POST.get("Nivel")
                ...
                if  nivel=='1':
                            literalNivel='Con Existencia en Cero'
                            if deposito=='':
                                producto = Producto.objects.filter(marca_id=marcaid,
                                Activo=Activoid,
                                tiene_iva=Tieneiva,
                                disponible=0)
                            else:
                            ...
                            literalNivel='Bajo Nivel Minimo'
                            if deposito=='':
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible <=p.MinUnidades')
                            else:
                                literalDeposito='Ubicacion ' +deposito
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible <=p.MinUnidades and p.Deposito= %s', [deposito])

                elif  nivel=='4':
                            literalNivel='Sobre Nivel Maximo'
                            if deposito=='':
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible >p.MaxUnidades')
                            else:
                                literalDeposito='Ubicacion ' +deposito
                                producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])


                marca = Marca.objects.filter(id=marcaid)
                contexto = {'producto':producto,
                            'marca':marca,
                            'literalactivo': literalactivo,
                            'literaliva':literaliva,
                ...
                RPdf.save()
                contexto = complementarContexto(contexto,request.user)
                return render(request, 'inventario/producto/ReporteInvMarca.html',contexto)
...
class Panel(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        contexto = {'usuario': request.user.username,
                   ...
                   'fecha':  date.today(),
                   'cateRegistradas' : Categoria.numeroRegistrados(),
                   'MrkRegistradas' : Marca.numeroRegistrados(),
                   'prodRegistrados' : Producto.numeroRegistrados(),
...
class listarCategorias(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, 'inventario/producto/listarCategorias.html',contexto)
#Fin de vista-------------------------------------------------------------------------#
class listarMarcas(LoginRequiredMixin, View):
    login_url = '/inventario/login'
    redirect_field_name = None
    ...
    def get(self, request):
        ...
        return render(request, 'inventario/producto/listarMarcas.html',contexto)
#Fin de vista-------------------------------------------------------------------------#
...
class ListarProductos(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # productos = Producto.objects.filter('select * from inventario_categoria , inventario_producto where inventario_categoria.id= inventario_producto.category_id')

        productos= Producto.objects.filter(Activo= True).values('id','category_id__nombrecatg',
                                                                                            'marca_id__NombreMarca',
                                                                                            'codbar',
                                                                                            'descripcion',
                                                                                            'precio',
                                                                                            'disponible',
...
#Fin de vista------------------------------------------------------------------------#
class AgregarMarcaProducto(LoginRequiredMixin, View):
    login_url = '/inventario/login'
    redirect_field_name = None
    ...
    def post(self, request):
        ...
        if form.is_valid():
            # Procesa y asigna los datos con form.cleaned_data como se requiere
            NombreMarca = form.cleaned_data['NombreMarca']

            MarcaProd = Marca(NombreMarca=NombreMarca)
            MarcaProd.save()

            form = MarcaFormulario()
            messages.success(request, 'Ingresado exitosamente bajo la ID %s.' % MarcaProd.id)
            request.session['MarcaProcesada'] = 'agregado'
            return HttpResponseRedirect("/inventario/AgregarMarcaProducto")
        else:
            #De lo contrario lanzara el mismo formulario
            return render(request, 'inventario/producto/agregarmarcaproducto.html', {'form': form})

    # Si se llega por GET crearemos un formulario en blanco
    def get(self,request):
        form = MarcaFormulario()
        #Envia al usuario el formulario para que lo llene
        contexto = {'form':form , 'modo':request.session.get('MarcaProcesada')}
        contexto = complementarContexto(contexto,request.user)
        return render(request, 'inventario/producto/agregarmarcaproducto.html', contexto)
#Fin de vista------------------------------------------------------------------------#
class AgregarCategoriaProducto(LoginRequiredMixin, View):
    login_url = '/inventario/login'
    redirect_field_name = None
    ...
    def post(self, request):
        ...
        if form.is_valid():
            # Procesa y asigna los datos con form.cleaned_data como se requiere
            nombrecatg = form.cleaned_data['nombrecatg']
            reference = form.cleaned_data['reference']
            disponible = 0

            CategoriaProd = Categoria(nombrecatg=nombrecatg,reference=reference,disponible=disponible)
            CategoriaProd.save()

            form = CategoriaFormulario()
            messages.success(request, 'Ingresado exitosamente bajo la ID %s.' % CategoriaProd.id)
            request.session['CategoriaProcesada'] = 'agregado'
            return HttpResponseRedirect("/inventario/AgregarCategoriaProducto")
        else:
            #De lo contrario lanzara el mismo formulario
            return render(request, 'inventario/producto/agregarcategoria.html', {'form': form})

    # Si se llega por GET crearemos un formulario en blanco
    def get(self,request):
        form = CategoriaFormulario()
        #Envia al usuario el formulario para que lo llene
        contexto = {'form':form , 'modo':request.session.get('CategoriaProcesada')}
        contexto = complementarContexto(contexto,request.user)
        return render(request, 'inventario/producto/agregarcategoria.html', contexto)
#Fin de vista------------------------------------------------------------------------#
...
class AgregarProducto(LoginRequiredMixin, View):
    ...
    def post(self, request):
     # Crea una instancia del formulario y la llena con los datos:
        ...
        if form.is_valid():
            # Procesa y asigna los datos con form.cleaned_data como se requiere
            codbar = form.cleaned_data['codbar']
            descripcionAmp= form.cleaned_data['descripcionAmp']
            ...
            MinUnidades = form.cleaned_data['MinUnidades']
            MaxUnidades = form.cleaned_data['MaxUnidades']
            marca = form.cleaned_data['marca']
            Activo = form.cleaned_data['Activo']

            prod = Producto(codbar=codbar,descripcionAmp=descripcionAmp,category=category,descripcion=descripcion,
            precio=precio,categoria=categoria,tiene_iva=tiene_iva,disponible=disponible,
                            Deposito=Deposito,MinUnidades=MinUnidades,MaxUnidades=MaxUnidades,marca=marca,Activo=Activo
                            )
            prod.save()
...
class EditarCategoria(LoginRequiredMixin, View):
    ...
    def post(self,request,p):
        # Crea una instancia del formulario y la llena con los datos:
        cat  = Categoria.objects.get(id=p)
        form = CategoriaFormulario(request.POST, instance=cat)
        # Revisa si es valido:
        if form.is_valid():
            # Procesa y asigna los datos con form.cleaned_data como se requiere
            nombrecatg  = form.cleaned_data['nombrecatg']
            ...
            catg = Categoria.objects.get(id=p)
            catg.nombrecatg = nombrecatg
            catg.reference = reference
            catg.save()
            form =CategoriaFormulario(instance=catg)
            messages.success(request, 'Actualizado exitosamente la categoria de ID %s.' % p)
            request.session['CategoriaProcesada'] = 'editado'
            return HttpResponseRedirect("/inventario/EditarCategoria/%s" % catg.id)
        else:
            #De lo contrario lanzara el mismo formulario
            return render(request, 'inventario/producto/agregarcategoria.html', {'form': form})
    ...
    def get(self, request,p):
        ...
        form =CategoriaFormulario(instance=catg)
        #Envia al usuario el formulario para que lo llene
        contexto = {'form':form , 'modo':request.session.get('CategoriaProcesada'),'editar':True}
        contexto = complementarContexto(contexto,request.user)
        return render(request, 'inventario/producto/AgregarCategoria.html', contexto)
#Fin de vista------------------------------------------------------------------------------------#
class EditarMarca(LoginRequiredMixin, View):
    ...
    def post(self,request,p):
        # Crea una instancia del formulario y la llena con los datos:
        mrk  = Marca.objects.get(id=p)
        form = MarcaFormulario(request.POST, instance=mrk)
        # Revisa si es valido:
        if form.is_valid():
            # Procesa y asigna los datos con form.cleaned_data como se requiere
            nombremarca  = form.cleaned_data['NombreMarca']
            ...
            mrk = Marca.objects.get(id=p)
            mrk.NombreMarca =nombremarca
            mrk.save()
            form =MarcaFormulario(instance=mrk)
            messages.success(request, 'Actualizado exitosamente la Marca de ID %s.' % p)
            request.session['MarcaProcesada'] = 'editado'
            return HttpResponseRedirect("/inventario/EditarMarca/%s" % mrk.id)
        else:
            #De lo contrario lanzara el mismo formulario
            return render(request, 'inventario/producto/agregarmarcaproducto.html', {'form': form})
    ...
    def get(self, request,p):
        ...
        form =MarcaFormulario(instance=mrk)
        #Envia al usuario el formulario para que lo llene
        contexto = {'form':form , 'modo':request.session.get('MarcaProcesada'),'editar':True}
        contexto = complementarContexto(contexto,request.user)
        return render(request, 'inventario/producto/agregarmarcaproducto.html', contexto)
#Fin de vista------------------------------------------------------------------------------------#
...
class EditarProducto(LoginRequiredMixin, View):
    ...
    def post(self,request,p):
        # Crea una instancia del formulario y la llena con los datos:
        prod  = Producto.objects.get(id=p)
        form = ProductoFormulario(request.POST, instance=prod)
        ...
        if form.is_valid():
            ...
            prod = Producto.objects.get(id=p)
            prod.codbar = codbar
            prod.descripcionAmp = descripcionAmp
            ...
            prod.Deposito = Deposito
            prod.MinUnidades =  MinUnidades
            prod.MaxUnidades =  MaxUnidades
            prod.marca=marca
            prod.Activo=Activo
            prod.save()
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            cliente = Cliente(cedula=cedula,nombre=nombre,apellido=apellido,
                ...
                correo=correo,telefono2=telefono2,correo2=correo2,activo=activo)
            cliente.save()
...
class GenerarFacturaPDF(LoginRequiredMixin, View):
    ...
    def get(self, request, p):
        ...
        #Fin de vista--------------------------------------------------------------------------------------#
class GenerarListadoCatProdPDF(LoginRequiredMixin, View):
    login_url = '/inventario/login'
    redirect_field_name = None
    ...
    def get(self, request):
            ...
            literalDeposito=''
            RPdf= ReporteProdPdf.objects.get(id=1)
            categoriarid=RPdf.categoriarid
            Activoid=RPdf.Activoid
            Tieneiva=RPdf.Tieneiva
            deposito=RPdf.deposito
            nivel=RPdf.nivel
            general = Opciones.objects.get(id=1)
            numtotal=0
            cursor=0
            ...
            if   nivel=='1':
                        ...
                        if deposito=='':
                            ...
                            tiene_iva=Tieneiva,
                            disponible=0)
                            numtotal = Producto.objects.filter(category_id=categoriarid,
                            Activo=Activoid,
                            tiene_iva=Tieneiva,
                            disponible=0).count()
                        else:
                        ...
                        if deposito=='':
                            ...
                            with connection.cursor() as cursor:
                                cursor.execute('SELECT count() from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible <=p.MinUnidades')
                                row = cursor.fetchone()
                                numtotal = row[0]
                        else:
                            literalDeposito='Ubicacion ' +deposito
                            producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible <=p.MinUnidades and p.Deposito= %s', [deposito])
                            with connection.cursor() as cursor:
                                  cursor.execute('SELECT count() from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible <=p.MinUnidades and p.Deposito= %s', [deposito])
                                  row = cursor.fetchone()
                                  numtotal = row[0]
            elif  nivel=='4':
                        literalNivel='Sobre Nivel Maximo'
                        if deposito=='':
                            producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible >p.MaxUnidades')
                            with connection.cursor() as cursor:
                                cursor.execute('SELECT count(*) from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible >p.MaxUnidades')
                                row = cursor.fetchone()
                                numtotal = row[0]
                        else:
                            literalDeposito='Ubicacion ' +deposito
                            producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])
                            numtotal = Producto.objects.raw('SELECT count() from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])
                            with connection.cursor() as cursor:
                                cursor.execute('SELECT count(*) from inventario_producto as p,inventario_categoria as c where p.category_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])
                                row = cursor.fetchone()
                                numtotal = row[0]
            ...
            nombre= "LstadoProductoporCategoria_%s.pdf" % (datetime.date.today())
            pdf = render_to_pdf('inventario/PDF/ReporteListCatProd.html', data)
            response = HttpResponse(pdf,content_type='application/pdf')
            response['Content-Disposition'] = 'attachment; filename="%s"' % nombre

            return response

class GenerarListadoMrkProdPDF(LoginRequiredMixin, View):
    login_url = '/inventario/login'
    redirect_field_name = None
    ...
    def get(self, request):
            ...
            if   nivel=='1':
                        ...
                        if deposito=='':
                            ...
                            tiene_iva=Tieneiva,
                            disponible=0)
                            numtotal = Producto.objects.filter(marca_id=marcaid,
                            Activo=Activoid,
                            tiene_iva=Tieneiva,
                            disponible=0).count()
                        else:
                        ...
                        if deposito=='':
                            producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible <=p.MinUnidades')
                            with connection.cursor() as cursor:
                                cursor.execute('SELECT count() from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible <=p.MinUnidades')
                                row = cursor.fetchone()
                                numtotal = row[0]
                        else:
                            literalDeposito='Ubicacion ' +deposito
                            producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible <=p.MinUnidades and p.Deposito= %s', [deposito])
                            with connection.cursor() as cursor:
                                  cursor.execute('SELECT count() from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible <=p.MinUnidades and p.Deposito= %s', [deposito])
                                  row = cursor.fetchone()
                                  numtotal = row[0]
            elif  nivel=='4':
                        literalNivel='Sobre Nivel Maximo'
                        if deposito=='':
                            producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible >p.MaxUnidades')
                            with connection.cursor() as cursor:
                                cursor.execute('SELECT count(*) from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible >p.MaxUnidades')
                                row = cursor.fetchone()
                                numtotal = row[0]
                        else:
                            literalDeposito='Ubicacion ' +deposito
                            producto = Producto.objects.raw('SELECT * from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])
                            numtotal = Producto.objects.raw('SELECT count() from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])
                            with connection.cursor() as cursor:
                                cursor.execute('SELECT count(*) from inventario_producto as p,inventario_marca as c where p.marca_id=c.id and p.disponible >p.MaxUnidades and p.Deposito= %s', [deposito])
                                row = cursor.fetchone()
                                numtotal = row[0]
            ...
            nombre= "LstadoProductoporMrca_%s.pdf" % (datetime.date.today())
            pdf = render_to_pdf('inventario/PDF/ReporteListMrkProd.html', data)
            response = HttpResponse(pdf,content_type='application/pdf')
            response['Content-Disposition'] = 'attachment; filename="%s"' % nombre
    ...
    'form-MAX_NUM_FORMS': '',
        ...
        if formset.is_valid():
            ...
            for form in formset:
                ...
                id_producto.append((desc)) #esta funcion, a estas alturas, es innecesaria porque ya tienes la id
                cantidad.append(cant)
                subtotal.append(sub)
...
class DescargarBDD(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        with fs.open('inventario_respaldo.xml','r') as output:
            ...
            #Borra el archivo
            ruta = 'inventario/archivos/inventario_respaldo.xml'
